...
//
// Project creation, Electric source creation, and `supabase link` remain one-time manual steps.
...
export function frontendCloudEnv(env: Record<string, string>): Record<string, string> {
  const url = requiredOneOf(
    env,
    ["VITE_GENRETV_SUPABASE_URL", "GENRETV_SUPABASE_URL"],
    "local cloud frontend Supabase URL",
  );
...
function requireSupabaseCli(): void {
  ...
  if (result.error || result.status !== 0) {
    throw new Error(
      `The Supabase CLI (\`${supabaseBin}\`) is required and is not runnable. Install it, run ` +
        "`supabase login` and `supabase link --project-ref <ref>`, or set SUPABASE_BIN. " +
...
function secrets(env: Record<string, string>): void {
  ...
  try {
    ...
    run(supabaseBin, ["secrets", "set", "--env-file", file]);
  } finally {
...
function functions(): void {
  ...
  run(supabaseBin, ["functions", "deploy", "genretv-write", "genretv-sync"]);
}
...
function main(): void {
  ...
  console.log(`GenreTV -> ${env["GENRETV_SUPABASE_URL"]}. Steps: ${steps.join(" -> ")}`);
