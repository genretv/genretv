...
export interface GenretvClaimsResolverOptions {
  supabaseUrl: string;
  logTimings?: boolean;
}

export function createGenretvClaimsResolver(options: GenretvClaimsResolverOptions) {
  ...
  return async function resolveGenretvClaims(request: Request): Promise<JwtClaims | null> {
